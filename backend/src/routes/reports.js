...
router.post('/submit', authenticate, requireRole('employee','manager','admin'), upload.single('file'), async (req, res) => {
  try {
  ...
  let { report_type_id, due_date, title, description, department_id, department_name, team_id, team_name, deliverable_id } = req.body;
    ...
    if (department_id !== undefined && department_id !== null && String(department_id).trim() !== '') {
      const parsedDept = parseInt(String(department_id), 10);
      if (!isNaN(parsedDept)) {
        const d = await Department.findByPk(parsedDept);
        if (d) resolvedDepartmentId = d.id;
      } else if (String(department_id).trim() !== '') {
        // treat as name
        let d = await Department.findOne({ where: { name: String(department_id).trim() } });
        if (!d && req.user && String(req.user.role).toLowerCase() === 'manager') {
          // manager may create a new department on submit
          d = await Department.create({ name: String(department_id).trim(), description: null });
        }
        if (d) resolvedDepartmentId = d.id;
      }
    } else if (department_name && String(department_name).trim() !== '') {
      let d = await Department.findOne({ where: { name: String(department_name).trim() } });
      if (!d && req.user && String(req.user.role).toLowerCase() === 'manager') {
    ...
    if (report_type_id !== undefined && report_type_id !== null && String(report_type_id).trim() !== '') {
      ...
      if (!isNaN(parsed)) {
        ...
        // try find by name
        const nameVal = String(report_type_id).trim();
        resolvedReportType = await ReportType.findOne({ where: { name: nameVal } });
        if (!resolvedReportType && req.user && String(req.user.role).toLowerCase() === 'manager') {
          // create report type and attach to resolved department if available
          const createDeptId = resolvedDepartmentId || null;
          resolvedReportType = await ReportType.create({ name: nameVal, department_id: createDeptId });
        }
    ...
    // If client provided a report_type_id but we couldn't resolve it (and user isn't manager who could create it), return a clear error
    if ((req.body.report_type_id !== undefined) && (report_type_id === null) && !(req.user && String(req.user.role).toLowerCase() === 'manager')) {
